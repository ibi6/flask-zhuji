...
/** 从 CSRF 响应体解析 token（mock 用 token，真实后端用 csrf_token） */
function parseCsrfPayload(data: { csrf_token?: string; token?: string }): string {
  const token = data.csrf_token ?? data.token;
  if (!token) throw new Error("CSRF 响应缺少 token 字段");
  return token;
}
...
export async function refreshCsrf(): Promise<void> {
  if (MOCK_ACTIVE) {
    ...
    setCsrfToken(parseCsrfPayload(data));
    return;
  }
  const res = await rawRequest("/auth/csrf", { method: "GET" });
  const data = (await res.json()) as { csrf_token?: string; token?: string };
  setCsrfToken(parseCsrfPayload(data));
}
