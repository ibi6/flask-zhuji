...
export interface AttackRegionStat {
  region: string;
  count: number;
}
...
export interface DashboardSummary {
  ...
  recent_alerts: Alert[];
  /** SOC 扩展指标（文档 §二，mock/新版 API 提供） */
  security_score?: number;
  agent_online_rate?: number;
  attack_events?: number;
  high_vulnerabilities?: number;
  abnormal_logins?: number;
  brute_force?: number;
  malicious_processes?: number;
  attack_trend_24h?: AttackTrendPoint[];
  attack_types?: AttackTypeStat[];
  top_attack_ips?: RankItem[];
  top_attacked_hosts?: RankItem[];
  attack_regions?: AttackRegionStat[];
  ai_insight?: AiInsight;
}

export type VulnFixStatus = "open" | "in_progress" | "fixed" | "accepted";
